...
class FixedFleeGhostAgent(GhostAgent):
	"""GhostAgent that always selects the flee behaviour."""

	def choose_action(self, state, action, reward, legal_actions, explore):
		"""Chooses an action according to the flee behaviour, always trying to keep itself the farthest away from the closest enemy
		...
		"""
		behaviour = behaviors.FleeBehavior()
		action = behaviour(state, legal_actions)
		if len(legal_actions) > 0:
			return action
		return Directions.STOP
...
class FixedSeekGhostAgent(GhostAgent):
	"""GhostAgent that always selects the seek behaviour."""

	def choose_action(self, state, action, reward, legal_actions, explore):
		"""Chooses an action according to the seek behaviour, trying to move itself into the position that it believes the enemy will move into
		...
		"""
		behaviour = behaviors.SeekBehavior()
		action = behaviour(state, legal_actions)
		if len(legal_actions) > 0:
			return action
		return Directions.STOP
...
class FixedPursueGhostAgent(GhostAgent):
	"""GhostAgent that always selects the pursue behaviour."""

	def choose_action(self, state, action, reward, legal_actions, explore):
		"""Chooses an action according to the pursue behaviour, trying to move itself into the position that it believes the enemy is
		...
		"""
		behaviour = behaviors.PursueBehavior()
		action = behaviour(state, legal_actions)
		if len(legal_actions) > 0:
			return action
		return Directions.STOP
